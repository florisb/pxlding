...
include ("class.image.config.unix.php");
...
/* properties */
  ...
  /* set the photo url */
    ...
    $this->sFileLocation = IMAGEBASEPATH . IMAGEDIRSEPARATOR . $this->filename;
    $this->sImageURL = IMAGEBASEURL . '/' . $this->filename;		
	}
  ...
  /* crop image (first resize with keep ratio) */
	...
	function writetext($sText, $iFontSize = 10, $sTextColor = '0,0,0', $sFontFilename = 'arial.ttf', $iXPos = 5, $iYPos = 15, $iTextAngle = 0) {
  /* write text on image */
    if (!$this->ImageStream) {
		  $this->printError('image not loaded'); 
    } 
