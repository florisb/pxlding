...
},{"../lib/dotdotdot/jquery.dotdotdot.min.js":1,"../lib/hammer/hammer.min.js":2,"../lib/typing/jquery.typing-0.2.0.min.js":3,"./modules/MainMenu.js":5,"./modules/Newsletter.js":6,"./settings.js":7}],5:[function(require,module,exports){
/**
 * Main Menu
 *
...
module.exports = (function() {
    ...
    $(function() {
        // general
        _initNewsLetterSubmit();
    });


    // Toggle tablet/phone filter container visibility

    /**
     * Catch submission of newsletter, do ajax call instead and show errors in form
     *
     * Always expects newsletter form to be the form inside a section.newsletter
     */
    var _initNewsLetterSubmit = function() {

        var data = {};

        $('section.newsletter form').submit(function(e) {
            //e.preventDefault();

            _clearErrors(this);

            data.geslacht   = $(this).find('input[name=geslacht]').val();
            data.voornaam   = $(this).find('input[name=voornaam]').val();
            data.emailadres = $(this).find('input[name=emailadres]').val();

            _doNewsletterSubmit(this, data);
     ...
     */
    var _doNewsletterSubmit = function(formElement, data) {

        data.ajax = 1;

        $.ajax({
            type     : 'POST',
            'url'    : $('base').attr('href') + 'newsletter/subscribe',
            'data'   : data,
            dataType : 'json',
            ...
            success: function(data) {
                ...
                if (data.hasOwnProperty('success') && data.success) {
                    window.location = $('base').attr('href') + 'newsletter/succes'; // note dutch spelling!
                    return;
                }
            ...
            error: function() {
                ...
                _showErrors(formElement, {
                    'general': 'Error, could not sumbit subscription. Please try again.'
                });
            }
        });
